...
class RequestRecievedClass extends Component {
    ...
    handleDelete = (r_id)=>{
        Axios.post('http://localhost:3001/deleteRequest', {r_id: r_id, iso: this.props.iso}).then(()=>{})
        console.log("Deleted", r_id)
    }

    render() { 
        Axios.post('http://localhost:3001/requestRecieved', {iso: this.props.iso}).then((response)=>{
            if (this.state.requestsData.length == 0){
                this.setState({requestsData: response.data, refresh: false})
            }
        })
        return (
            <React.Fragment>
